...
def ss_with_inputs(dt, start_date, end_date):
    """
    ...
    return TC, As, Bs, Cs, Ds, u, input_data_set


def qHVAC(Cs, Ds, θ_exp, Kp):
    # outputs
...
def plot_results(input_data_set, u, y):
    # plot
...
TC, As, Bs, Cs, Ds, u, input_data_set = ss_with_inputs(dt, start_date, end_date)
...
for k in range(1, u.shape[0] - 1):
    if Tisp + Δθ < θ_exp.iloc[k - 1]['c2_θ0']:
        ...
        u.iloc[k]['c2_θ0'] = max(u.iloc[k]['c2_θ0'], 0)
        # u.iloc[k]['c2_θ0'] = max(u.iloc[k]['c2_θ0'], 700)
...
qHVAC = qHVAC(Cs, Ds, θ_exp, Kpp)
plot_results(input_data_set, u, y)
